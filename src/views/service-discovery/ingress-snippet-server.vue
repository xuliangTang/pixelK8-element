  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span>自定义配置(server)</span>
      </div>
      <div>
        <el-form label-width="150px">
          <el-form-item label="启用">
            <el-switch v-model="subConfig.server_snippet_enable" />
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  methods: {
    initEditor() {
      ...
      this.editor.setValue(this.subConfig.server_snippet)
      this.editor.on('change', (editor, changes) => {
        this.subConfig.server_snippet = this.editor.getValue()
      })
    },
    output() {
      const result = {}
      if (this.subConfig.server_snippet_enable) {
