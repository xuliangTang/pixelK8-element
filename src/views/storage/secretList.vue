        ...
        >
          ...
          <el-table-column label="名称">
            <template slot-scope="scope">
              <router-link :to='"secret-show?ns=" + scope.row.namespace + "&name=" + scope.row.name'><el-link type="primary">{{ scope.row.name }}</el-link></router-link>
            </template>
          </el-table-column>
...
<script>
import { getSecretList, deleteSecret } from '@/api/secret'
...
export default {
  ...
  methods: {
    handleMore(command) {
      if (command[0] === 'drop') {
        deleteSecret(command[1].namespace, command[1].name).catch((err) => {
          if (err.response) {
